/*jshint -W065 */
var blossompinRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['blossom-pin-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new blossompinRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'blossom_pin_get_fontawesome_ajax',
					blossom_pin_customize_nonce: blossom_pin_customize.nonce
				},
				beforeSend: function(){
