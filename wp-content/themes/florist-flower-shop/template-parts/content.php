...
<div id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="post-main-box p-3 mb-3 wow bounceInUp delay-1000" data-wow-duration="2s">
    <?php $florist_flower_shop_theme_lay = get_theme_mod( 'florist_flower_shop_blog_layout_option','Default');
    if($florist_flower_shop_theme_lay == 'Default'){ ?>
      <div class="row">
        <?php if(has_post_thumbnail() && get_theme_mod( 'florist_flower_shop_featured_image_hide_show',true) == 1) {?>
          <div class="box-image col-lg-6 col-md-6">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
        <article class="new-text <?php if(has_post_thumbnail() && get_theme_mod( 'florist_flower_shop_featured_image_hide_show',true) == 1) { ?>col-lg-6 col-md-6" <?php } else { ?>col-lg-12 col-md-12" <?php } ?>>
          ...
          <?php if( get_theme_mod( 'florist_flower_shop_toggle_postdate',true) == 1 || get_theme_mod( 'florist_flower_shop_toggle_author',true) == 1 || get_theme_mod( 'florist_flower_shop_toggle_comments',true) == 1 || get_theme_mod( 'florist_flower_shop_toggle_time',true) == 1) { ?>
            <div class="post-info p-2 my-3">
              <?php if(get_theme_mod('florist_flower_shop_toggle_postdate',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_toggle_postdate_icon','fas fa-calendar-alt')); ?>"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $florist_flower_shop_archive_year, $florist_flower_shop_archive_month, $florist_flower_shop_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span><?php echo esc_html(get_theme_mod('florist_flower_shop_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('florist_flower_shop_toggle_author',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_toggle_author_icon','fas fa-user')); ?>"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span><span><?php echo esc_html(get_theme_mod('florist_flower_shop_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('florist_flower_shop_toggle_comments',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_toggle_comments_icon','fa fa-comments')); ?>" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'florist-flower-shop'), __('0 Comments', 'florist-flower-shop'), __('% Comments', 'florist-flower-shop') ); ?></span><span><?php echo esc_html(get_theme_mod('florist_flower_shop_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('florist_flower_shop_toggle_time',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('florist_flower_shop_toggle_time_icon','fas fa-clock')); ?>"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
              <?php } ?>
    ...
    <?php }else if($florist_flower_shop_theme_lay == 'Center'){ ?>
      <div class="service-text">
        <h2 class="section-title mt-0 pt-0"><a href="<?php the_permalink(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
        <?php if( get_theme_mod( 'florist_flower_shop_featured_image_hide_show',true) == 1) { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
    ...
    <?php }else if($florist_flower_shop_theme_lay == 'Left'){ ?>
      <div class="service-text">
        <?php if( get_theme_mod( 'florist_flower_shop_featured_image_hide_show',true) == 1) { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
